...
function updateCountdown() {    // 140 is the max message length
...
//$(document).on("page:change", function() {
 // timelyrefreshStories = function(){
    // when post, save Time - to what? db? variable?
 // Every x seconds
    // "SELECT * FROM `microposts` WHERE `created_at` >= Time" 
    // if a micropost is returned
    // display in container
// how to get last micropost as a url...  or as data???
 //    if feed-item-created-at > Time
 //      get the id of the last micropost
//              alert('timely micropost');
   //       $.ajax({
     //       type: "GET",
       //     url: "/stories/"+$('.feedsentence').data('story-id'),   // doesnt work in mp.js
//            url: "/microposts/"+$('.feedsentence').data('feed-item-id'), 
 //           url: "/stories/"+$('.partialfeed').data('story-id')+"/edit",   // what is being gotten
           // if I use  url: /microposts/id , the micropost is only thing on page thus there's no feed_item divs & id is undefined
           // url: "http://google.com" this does nothing cuz it's external site
           // url: "/stories"
           // data: "{$('.feedsentence').data('feed-item-id')}",  // this makes site do nothing   
     //       dataType: "text"
       // }).done(function(result) {  //result is the html from url
//              alert('done fcn' +$('.partialfeed').data('story-id'));
         //   var $sentence_requests = $('.postfeed');   //where it's being put
//              alert('sentence ' + $('.microform').data('created-at'));
           // if ($sentence_requests.length > 0) {
 //             alert('jquery ' + jQuery('#micropost_content').val() );  //this is only for local textfield while typing 
 //            alert(result);
                    
             // if($('.postfeed').data('created_at) > Time)  
             // process result (query?) to get last micropost
//              if($('.feedsentence').data('feed-item-id') > 86) {  //only gets id for 1st micropost
          //        $sentence_requests.html(result);    //result is being put in sentencereq (postfeed)
//                  $sentence_requests.html($('.feedsentence').data('feed-item-id'));    //result is being put in sentencereq (postfeed)
  //              alert('UPDATED ' + $('.feedsentence').data('feed-item-created-at'));
 //             }  
     //       } else {
      //          alert('CANNOT access micropost requests container');
     //       }
           // data: "{'hi!'}",  // this makes site do nothing   
          //  var $sentence_requests = $('.postfeed');   //where it's being put
//                alert('UPDATED ' +$('.feedsentence').data('story-id') + $('.feedsentence').data('feed-item-id'));
           // if ($sentence_requests.length > 0) {
          //    alert('length ' + $('.feedsentence').data('feed-item-created-at') + "   " + $('.feedsentence').data('feed-item-id'));
             // if($('.postfeed').data('created_at) > Time)  
            //  if($('.feedsentence').data('feed-item-id') > 86) {  
            //    $sentence_requests.html(result);
           //     alert('UPDATED micropost requests ' + $('.feedsentence').data('feed-item-created-at'));
           //   }  
     //   }).append;
 // };

 //         $( "ol" ).append( $('.feedsentence').data('feed-item-content') );

   //     });   //.append does nothing here
//       $('.storyfeed').append( $('.microform') );
//  };
...
//var update;
//update = function() {
 // return $.get('/microposts.json', null, function(data, status, jqXHR) {
//  return $.get('/microposts.json', null, function(data, status, jqXHR) {
//    return $.get('/stories/"+$('.microform').data('story-id')' {
      //          alert("getjson " + $.get('/microposts.json'));
     //           alert("data " + data + "s " + status + " jx " + jqXHR);
   //     $( ".postfeed" ).append( $('.feedsentence').data('feed-item-content') + " return!!");
 //   var d, m, msgsSorted, _i, _len;
  //              alert("mpost " + micropost);
 //   msgsSorted = _.sortBy(data, function(micropost) {
 //               alert("msgsort " + msgsSorted);
 //               alert("mpost2 " + micropost);
  //    return new Date(micropost.created_at).getTime();
 //   });
 //   $('.postfeed').empty();
 //   for (_i = 0, _len = msgsSorted.length; _i < _len; _i++) {
 //     m = msgsSorted[_i];
 //     d = moment(m.created_at);
 //     $('.postfeed').append('<li>' + '<span class="created_at">' + d.format('hh:mm') + '</span>' + m.content + '</li>');
 //   }
...
//! moment.js
//! momentjs.com
//not sure about this weird moment function
...
(function() {
  ...
  update = function() {
    alert("1st coffee");
    return $.get('/stories/feed.json', null, function(data, status, jqXHR) {
      var d, m, msgsSorted, _i, _len;
      ...
      for (_i = 0, _len = msgsSorted.length; _i < _len; _i++) {
        ...
        d = moment(m.created_at);
        $('ol').append("hi");
        $('.postfeed').append('<li>' + "hi" + '<span class="created_at">' + d.format('hh:mm') + '</span>' + m.content + '</li>');
      }
      return setTimeout(update, 750);
    });
  };
...
(function() {function async_load(){s.id="cid0020000058590817580";s.src='http://st.chatango.com/js/gz/emb.js';s.style.cssText="width:200px;height:300px;";s.async=true;s.text='{"handle":"sentencezing","arch":"js","styles":{"a":"CC0000","b":100,"c":"FFFFFF","d":"FFFFFF","k":"CC0000","l":"CC0000","m":"CC0000","n":"FFFFFF","q":"CC0000","r":100,"pos":"bl","cv":1,"cvfntsz":"20px","cvbg":"CC0000","cvw":123,"cvh":45}}';var ss = document.getElementsByTagName('script');for (var i=0, l=ss.length; i < l; i++){if (ss[i].id=='sid0020000058590817580'){ss[i].id +='_';ss[i].parentNode.insertBefore(s, ss[i]);break;}}}var s=document.createElement('script');if (s.async==undefined){if (window.addEventListener) {addEventListener('load',async_load,false);}else if (window.attachEvent) {attachEvent('onload',async_load);}}else {async_load();}})();
(function() {
...
//    $('.storyfeed').live 'ajax:success', (event,data) -> $('.storyfeed').html(data.html) if(data.success == true)
   
    //    $.ajax({
    //        type: "GET",
//            url: "/stories/"+$('.storyfeed').data('story-id')
    //        url: "/stories/"+$('.storyfeed').data('story-id')   // what is being gotten
  //      }).done(function(result) {
   //         var $sentence_requests = $('.storyfeed');
//                alert('doneStories' );
   //         if ($sentence_requests.length > 0) {
   //             $sentence_requests.html(result);
//                alert('UPDATED micropost requests ' + $('.storyfeed').data('story-id'));
  //          } else {
    //            alert('CANNOT access micropost requests container');
   //         }
   //     })
//       $('.dummyappend').append( $('.postbutton') );  //textbox doesnt appear in other user's browser
//      $('.storyfeed').append( $('.microform') );    //textfield disappears upon autorefresh
//    };
//  setInterval(timelyrefreshStories, 18000);
//});  

  //json array of ids
           // url: "/stories/"+$('.storyfeed').data('story-id')+"/edit"
;
(function() {
  var subscription;
